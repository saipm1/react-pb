...
const INVALID_SSL_CERTIFICATE_DETAILS =
  "Invalid SSL Certificate Env. Details Specified!";

class MainClass {
  static main(): void {
    try {
      const portNumber =
        parseInt(process.env.LISTENER_PORT || "") || DEFAULT_PORT_NUMBER;
      ...
      host.start().then(
        () => LogManager.info("Customer Service Host Started Successfully!"),
