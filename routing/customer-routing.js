...
const customer_1 = require("../business/services/customer");
...
class CustomerRouting {
    constructor(customerService) {
        this.customerService = customerService || new customer_1.CustomerService();
    ...
    initializeRouting() {
        this.router.get("/", (request, response) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (customerRecords) {
                    response.status(constants_1.HttpStatusCodes.OK).send(customerRecords);
                }
                else {
                    response.status(constants_1.HttpStatusCodes.NO_CONTENT).send({
                        message: NO_RECORDS_FOUND,
                    });
                }
            }
            catch (exception) {
                response.status(constants_1.HttpStatusCodes.SERVER_ERROR).send(exception);
            }
        }));
        this.router.get("/:customerId", (request, response) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (filteredRecord) {
                    response.status(constants_1.HttpStatusCodes.OK).send(filteredRecord);
                }
                else {
                    response.status(constants_1.HttpStatusCodes.NO_CONTENT).send({
                        message: NO_RECORDS_FOUND,
        ...
        this.router.get("/search/:searchString", (request, response) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (filteredRecords !== null && filteredRecords.length >= MIN_RECORDS) {
                    response.status(constants_1.HttpStatusCodes.OK).send(filteredRecords);
                }
                else {
                    response.status(constants_1.HttpStatusCodes.NO_CONTENT).send({
                        message: NO_RECORDS_FOUND,
        ...
        this.router.post("/", (request, response) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (status) {
                    response.status(constants_1.HttpStatusCodes.OK).send(status);
                }
                else {
                    response.status(constants_1.HttpStatusCodes.SERVER_ERROR).send({
