import {
  CustomerService,
  ICustomerService,
} from "../business/services/customer";
  ...
  private initializeRouting() {
    this.router.get("/", async (request, response) => {
      try {
        ...
        if (customerRecords) {
          response.status(HttpStatusCodes.OK).send(customerRecords);
        } else {
        ...
        response.status(HttpStatusCodes.SERVER_ERROR).send(exception);
      }
    });
    ...
    this.router.get("/:customerId", async (request, response) => {
      ...
      try {
        const filteredRecord = await this.customerService.getCustomerById(
          customerId
        );

        if (filteredRecord) {
          response.status(HttpStatusCodes.OK).send(filteredRecord);
        } else {
          response.status(HttpStatusCodes.NO_CONTENT).send({
            message: NO_RECORDS_FOUND,
          });
    ...
    this.router.get("/search/:searchString", async (request, response) => {
      ...
      const validation =
        searchString && searchString.length >= MIN_SEARCH_STR_LEN;
      ...
      try {
        const filteredRecords = await this.customerService.searchCustomers(
          searchString
        );

        if (filteredRecords !== null && filteredRecords.length >= MIN_RECORDS) {
          response.status(HttpStatusCodes.OK).send(filteredRecords);
        } else {
          response.status(HttpStatusCodes.NO_CONTENT).send({
            message: NO_RECORDS_FOUND,
          });
    ...
    this.router.post("/", async (request, response) => {
      ...
      try {
        const status = await this.customerService.addNewCustomer(
          customerRecord
        );
        ...
        if (status) {
          ...
          response.status(HttpStatusCodes.SERVER_ERROR).send({
